     ...
     int		douniq;
{
  ...
  int			clen;
  static unsigned	classmask[] = { /* ���ˤĤʤ��륯�饹 */
  ...
  for (clen = 0; (clen <= maxclen && clen < ye); clen++) {
    ...
    /* �ɤߤ�Ĺ�� clen ��ñ��Τ��������� class �ǻ��ꤵ�줿ñ�줬
       �Ĥʤ����ǽ���������Τ�ꥹ�ȥ��åפ���tail �˵�Ͽ���� */
    for (p = xqh[clen], sameLen = 0; p; p = p->nw_next) {
      if (classmask[p->nw_class] & (1<<class)) {
	/* p �θ��� class ��ñ�줬�Ĥʤ����ǽ�������� */
	if (sameLen < TAILSIZE) { /* �ޤ� tail �ˤ��������� */
	  tail[sameLen++] = p;
	}
      }
    }
     ...
     struct splitParm	*ul;
{
  ...
  {
    if (!CanSplitWord(p) || /* ʸ��ˤʤ�ʤ� */
	OnlyBunmatu(p) || /* ��ƥ���ľ���Ǥ���ʸ��ˤʤ�ʤ� */
	(p->nw_rowcol == cx->gram->P_KJ) || /* ñ���� */
	(p->nw_flags & NW_LOWPRI) || /* ͥ���٤��㤤ʸ�� */
	(p->nw_flags & NW_SUC))
      continue;
    if (l2 <= p->nw_ylen) {
      l2 = p->nw_ylen;
